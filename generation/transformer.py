...
class LMModel(nn.Module):
    ...
    def generate(self, prefix, max_len=100, beam_size=None):
        ...
        if beam_size == None:
            beam_size = 1
        def embedd(index):
            return torch.tensor([index]).reshape(1,1).to(x.device)#.cuda()
        bests = [(0,outputs,x,False)]
        # data structure:   
        ...
        for round in range(1,max_len):
            ...
            for prob,outputs,history,done in bests:
                ...
                logits = F.log_softmax(logits,dim=-1)[0,-1] # this is the log prob
...
class Seq2SeqModel(nn.Module):
    ...
    def generate(self, inputs, max_len=100, beam_size=None):
        ...
        inputs = torch.cat((torch.tensor([self.dictionary.bos()]),self.dictionary.encode_line(inputs))).reshape(1,-1).to(torch.long).cuda()
        if pad_len > 0:
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            key = key * (1-key_padding_mask.to(torch.float)).transpose(0,1).unsqueeze(2)
        ...
        eij_new = F.dropout(eij_new,p=self.dropout,training=self.training)
