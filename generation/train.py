...
def get_args():
    ...
    parser.add_argument("--start-epoch", default=0, type=str)
...
def train(args):
    ...
    if args.load is not None:
        model = torch.load(args.load,map_location='cpu').to(device)
        ...
        with tqdm(train_loader, desc="training") as pbar:
            ...
            for samples in pbar:
                ...
                pbar.set_description("Epoch: %d, Loss: %0.8f, lr: %0.6f" %
                                     (epoch + 1 + args.start_epoch, np.mean(losses),
                                      optimizer.param_groups[0]['lr']))

        if (epoch+args.start_epoch) % args.save_interval == 0:
            torch.save(
                model,
                args.save_dir + "/{}_{}.pt".format(args.model_type, epoch + 1 + args.start_epoch))
        evaluate(model, valid_set)
